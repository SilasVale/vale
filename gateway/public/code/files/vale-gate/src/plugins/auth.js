...
import { createUser, getUser, findUserByUsername, regenerateToken, getUserKeys, setUserKey, deleteUserKey, getAdminPassword, hasAdminPassword, verifyAdminPassword, maskKey, ADMIN_ID, USER_KEY_NAMES, getUserRoute, setUserRoute, getGlobalSetting, setGlobalSetting } from "../store.js";
...
import { MODELS, OG_ZEN_CHAT } from "../channels.js";
import { jsonOk, jsonError, readJson } from "../http.js";
...
async function authLogout(request, env, secure) {
...
async function meGetRoute(request, env) {
  const user = await requireSession(request, env);
  if (!user) return jsonError(401, "Not logged in or session expired", "authentication_error");
  return jsonOk({ model: await getUserRoute(env, user.id) });
}

async function mePutRoute(request, env) {
  const user = await requireSession(request, env);
  if (!user) return jsonError(401, "Not logged in or session expired", "authentication_error");
  const body = await readJson(request);
...
async function meRegenerateToken(request, env) {
  const user = await requireSession(request, env);
  if (!user) return jsonError(401, "Not logged in or session expired", "authentication_error");
...
async function meGetUsproxy(request, env) {
  const user = await requireSession(request, env);
  if (!user) return jsonError(401, "Not logged in or session expired", "authentication_error");
...
async function mePutUsproxy(request, env) {
  const user = await requireSession(request, env);
  if (!user) return jsonError(401, "Not logged in or session expired", "authentication_error");
  if (user.role !== "admin") {
...
async function mePutKeys(request, env) {
  const user = await requireSession(request, env);
  if (!user) return jsonError(401, "Not logged in or session expired", "authentication_error");
  const body = await readJson(request);
...
async function meDeleteKeys(request, env, url) {
  const user = await requireSession(request, env);
  if (!user) return jsonError(401, "Not logged in or session expired", "authentication_error");
...
async function meTestKeys(request, env) {
  const user = await requireSession(request, env);
  if (!user) return jsonError(401, "Not logged in or session expired", "authentication_error");
  const body = await readJson(request);
...
export default {
  ...
  setup(ctx) {
    ...
    const withSecure = (fn) => (request, env, url) => fn(request, env, url.protocol === "https:");
