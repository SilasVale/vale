 ...
 */
import { hasRegKey, hasRegGrant, getDevice, upsertDevice, deleteDevice, deleteRegKey, deleteRegGrant, consumeRegKey, getCfToken, getAdminPassword, getUser, maskKey, listDevices, addPluginLink, getPluginByToken, removePluginLink, consumePairCode, createWsTicket, consumeWsTicket } from "../store.js";
import { parseCookie, SESSION_COOKIE, verifySessionToken, randomHex } from "../auth.js";
...
async function handleTunnelToken(request, env) {
  const body = await readJson(request);
...
async function handleDeviceProxy(request, env, url) {
...
async function handleDevicesAdd(request, env) {
  const user = await requireSession(request, env);
  if (!user) return jsonError(401, "Not logged in or session expired", "authentication_error");
  if (user.role !== "admin") return jsonError(403, "Admin permission required", "authorization_error");
  const body = await readJson(request);
...
async function handleDeviceMcp(request, env, url) {
  const user = await requireSession(request, env);
  if (!user) return jsonError(401, "Not logged in or session expired", "authentication_error");
  if (user.role !== "admin") return jsonError(403, "Admin permission required", "authorization_error");
  const path = url.pathname;
...
async function handleDeviceDelete(request, env, url) {
  const user = await requireSession(request, env);
  if (!user) return jsonError(401, "Not logged in or session expired", "authentication_error");
  if (user.role !== "admin") return jsonError(403, "Admin permission required", "authorization_error");
  const path = url.pathname;
