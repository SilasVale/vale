...
import { getCfToken, setCfToken, createInvite, listUsers, getUserKeys, setUserEnabled, hasAdminPassword, setAdminPassword, verifyAdminPassword, getAdminPassword, getUser, maskKey, ADMIN_ID, USER_KEY_NAMES } from "../store.js";
import { parseCookie, verifySessionToken, SESSION_COOKIE } from "../auth.js";
import { MODELS, ROUTE_INFO } from "../channels.js";
...
async function adminPublic(request, env) {
...
async function adminGetCfToken(request, env) {
...
async function adminPutCfToken(request, env) {
  const user = await requireSession(request, env);
  if (!user || user.role !== "admin") return jsonError(401, "Not logged in", "authentication_error");
...
async function adminSetUserEnabled(request, env, url) {
  const user = await requireSession(request, env);
  if (!user || user.role !== "admin") return jsonError(401, "Not logged in", "authentication_error");
...
async function adminGetPassword(request, env) {
  const user = await requireSession(request, env);
  if (!user || user.role !== "admin") return jsonError(401, "Not logged in", "authentication_error");
  return jsonOk({ set: await hasAdminPassword(env) });
}

async function adminPutPassword(request, env) {
  const user = await requireSession(request, env);
  if (!user || user.role !== "admin") return jsonError(401, "Not logged in", "authentication_error");
  const body = await readJson(request);
