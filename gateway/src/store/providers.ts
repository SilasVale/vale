...
export async function providerForPrefix(
  env: Env,
  prefix: string,
...
export async function advertisedProviderModels(env: Env): Promise<AdvertisedProviderModel[]> {
  ...
  for (const p of await readList(env)) {
    ...
    for (const m of models) {
      const wire = String(m?.id ?? "").trim().replace(/^\/+/, "");
      if (!wire) continue;
...
export function parseProviderSpec(body: any): { spec?: ProviderSpec } & Partial<ProviderParseError> {
  ...
  if (label.length > 80 || /[\u0000-\u001f\u007f]/.test(label)) {
    return { error: "label must be at most 80 characters, with no control characters", status: 400 };
  }
  ...
  for (let i = 0; i < body.models.length; i++) {
    ...
    if (models.some((m) => m.id === wire)) {
      return { error: `models[${i}]: duplicate model id ${advertisedModelId(bare, wire)}`, status: 400 };
    }
