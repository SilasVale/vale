...
async function handleConsole(request: Request, env: any, url: URL) {
  ...
  if (method === "POST" && path === `${AUTH_BASE}/register`)
    return authRegister(request, env, secure);
  ...
  if (proxyMatch) {
    ...
    if (link && link.device === deviceName) {
      ...
      if (qToken && isNav) {
        ...
        resp.headers.append(
          "Set-Cookie",
          `vale_pt_${deviceName}=${encodeURIComponent(qToken)}; Path=${DEVICE_BASE}/${deviceName}/proxy; HttpOnly; Secure; SameSite=Lax; Max-Age=2592000`,
        );
      }
      return resp;
    }
  ...
  if (method === "PUT" && path.startsWith(`${ADMIN_BASE}/users/`) && path.endsWith("/enabled")) {
    ...
    const body = await readJson(request);
    if (id === ADMIN_ID)
      return jsonError(400, "Cannot disable the admin account", "invalid_request");
...
export async function handleGateway(request: Request, env: any, url: URL) {
  ...
  return new Response(
    JSON.stringify({
      type: "error",
      error: { type: "api_error", message: "translate plugin unavailable" },
    }),
    {
      status: 500,
...
async function cachedDeviceProbe(env: any, device: any) {
  ...
  const state = { tunnel: false, agent: false, ts: Date.now() };
  try {
    const res = await deviceFetch(env, device, "/api/status");
    state.tunnel = true;
    state.agent = res.ok;
  } catch {
    /* tunnel-level failure (1033/530/network) */
  }
...
function pickRoute(prefix: string, env: any, usProxy?: string | null) {
  ...
  const via = (direct: string, path: string) =>
    usProxy
  ...
  switch (prefix) {
    case "or":
      return {
        ...
        upstream: via(
          (env.OPENROUTER_PROXY_URL || "https://v.saisi.online/api/proxy") + VERIFY_PATH,
          "/v1/messages",
        ),
      };
    case "ds":
