...
const I18N = {
  zh: {
    ...
    "devices.installStep2": "2. 可选 — 让设备出现在本列表（联网登记）：生成注册码（可复制），然后在设备页面 Settings → Gateway 卡片里粘贴并保存。",
    "devices.genKey": "生成注册码",
    "devices.keyGenerated": "注册码已生成（一次性，1 小时有效）：{code}",
    "devices.keyCopied": "注册码已复制",
    "devices.genKeyFail": "生成失败（需要管理员权限）",
    "devices.installNote": "本地安装已完成即全部功能可用（终端/浏览器/记忆/面板，零云依赖）。公网访问（隧道）或联网登记均为可选，装好后在设备 Settings → Gateway 卡片配置。",
  ...
  en: {
    ...
    "devices.installStep2": "2. Optional — make the device appear in this list (online registration): generate a key (copyable), then paste it into the device's Settings → Gateway card and save.",
