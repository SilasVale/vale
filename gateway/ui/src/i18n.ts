...
const I18N = {
  zh: {
    ...
    "route.desc": 'Claude Code 模型名配 <code>auto</code> 后，本账号的请求就按下面选中的渠道走。切换在<strong>模型目录</strong>里点选，即时生效、无需重启；这里只显示当前选中的是哪一个。未选择时默认走 <code>cm/deepseek/deepseek-v4.1-flash</code>。',
    "route.use": "使用",
    "route.current": "当前",
    "route.bad": "异常",
