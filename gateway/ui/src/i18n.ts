...
const I18N = {
  zh: {
    ...
    "nav.routes": "模型路由",
    "nav.models": "模型目录",
    "models.lede": "本网关对外广告的全部模型，按渠道分组。点击任一模型即可把本账号的路由切到它——列表由服务端注册表派生，不在这里手写。",
    "models.count": "个模型",
    "models.up": "渠道可用",
    "models.down": "渠道异常",
    ...
    "routes.title": "路由状态",
    "routes.lede": '请求模型名按 <code>前缀/模型</code> 路由到对应后端；无前缀默认走 Command Code（模型名原样透传）。',
    "route.title": "渠道切换",
    "route.desc": 'Claude Code 模型名配 <code>auto</code> 后，在这里点一下即可切换，无需重启。未选择时默认走 <code>cm/deepseek/deepseek-v4.1-flash</code>。',
    "route.use": "使用",
    "route.current": "当前",
    "route.bad": "异常",
    ...
    "usproxy.title": "美国出口",
    "usproxy.desc": "所有模型经 Vercel 美国节点访问上游，规避区域限制。注意：og/deepseek-v4.1-flash 的慢在 zen 服务器自身，此开关对它无改善。",
  ...
  en: {
    ...
    "routes.lede": "Model names are routed by prefix; no prefix defaults to Command Code (the model name passes through as-is).",
    "route.title": "Channel switch",
    "route.desc":
      "Set the Claude Code model to <code>auto</code>, then flip channels here — no restart needed. Defaults to <code>cm/deepseek/deepseek-v4.1-flash</code> when unset.",
    "route.use": "Use",
