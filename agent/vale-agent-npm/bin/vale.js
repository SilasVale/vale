...
function resolveDir() {
  ...
  try {
    const out = (0, child_process_1.spawnSync)(
      "reg",
...
function sh(cmd, opts = {}) {
  return (0, child_process_1.spawnSync)(cmd, {
    shell: true,
    stdio: "inherit",
    ...opts,
  });
...
function ps(script) {
  ...
  return (0, child_process_1.spawnSync)("powershell", psArgv(script), {
    stdio: "inherit",
  });
}
...
function psFile(script) {
  ...
  try {
    ...
    return (0, child_process_1.spawnSync)(
      "powershell",
...
function latestCdnVersion() {
  ...
  const r = (0, child_process_1.spawnSync)(
    "curl",
    ["-s", "-m", "3", `${base}/api/version`],
...
async function awaitReleaseMarker(o) {
  ...
  for (;;) {
    try {
      ...
      saw = v || null;
      if (saw === o.want) return { ok: true, saw };
    } catch {
      saw = null; // absent / unreadable is NOT success and NOT a crash
    }
    if (o.now() >= deadline) return { ok: false, saw };
...
function releaseMarkerVerdict(c) {
  ...
  return {
    ...
    exitCode: 1,
    message:
      `rollback: the swap did NOT take -- device is on ${c.saw ?? "an unknown version"}, ` +
...
function initTunnel(hostname, regKey) {
  ...
  const r1 = (0, child_process_1.spawnSync)(
    cf,
  ...
  (0, child_process_1.spawnSync)(cf, ["tunnel", "create", name], {
    stdio: "inherit",
  });
  const list =
    (0, child_process_1.spawnSync)(cf, ["tunnel", "list", "--name", name], {
      encoding: "utf8",
    }).stdout || "";
  ...
  const r3 = (0, child_process_1.spawnSync)(
    cf,
...
const commands = {
  ...
  setup(args) {
    ...
    if (regKey) {
      ...
      // The CLI does not write config.yaml at all; the AGENT creates it on first
      // boot from its embedded default, which sets
      // platform.console_url = https://api.saisi.online, and main.rs then POSTs
      // {name, hostname, token} to /api/devices/self-register at boot and every
      // 6h. So a no-key install DOES contact the cloud and appear in the console —
      // which is what the very next lines already say ("device registers on
      // start"), so the operator was told both things at once.
      // The URL is NOT repeated here on purpose: it lives in the agent's embedded
      // default config, and a second copy in a CLI message is a copy that drifts
      // (the round-65 lesson, one surface over).
      console.log(
    ...
    try {
      const mig = psFile(
        migrateLayoutPs((0, exports.psq)(DIR), (0, exports.psq)(DATA_DIR)).join(
          "\r\n",
        ),
      );
      if (!mig || mig.status !== 0)
    ...
    sh(
      `powershell -NoProfile -Command "Remove-Item -Force -ErrorAction SilentlyContinue '${(0, exports.psq)(BUSY)}'"`,
    );
    ...
    sh(
      `powershell -NoProfile -Command "Get-CimInstance Win32_Process | Where-Object { $_.Name -eq 'node.exe' -and $_.CommandLine -like '*${(0, exports.psq)(DIR)}*playwright*' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }"`,
    );
    sh(
      `powershell -NoProfile -Command "Remove-Item -Recurse -Force -ErrorAction SilentlyContinue '${(0, exports.psq)(PW_DIR)}'"`,
    );
    ...
    for (const legacy of ["C:\\vale-agent", "D:\\vale-agent"]) {
      if (legacy !== DIR && fs.existsSync(legacy)) {
        ...
        sh(
          `powershell -NoProfile -Command "Remove-Item -Recurse -Force -ErrorAction SilentlyContinue '${(0, exports.psq)(legacy)}'"`,
        );
      }
    }
    ...
    sh(
      `powershell -NoProfile -Command "${deskShortcutRepairPs(
        (0, exports.psq)(SCRIPTS_DIR),
        (0, exports.psq)(DESK_DIR),
        "Write-Host",
      )
        .join("; ")
        .replace(/"/g, '\\"')}"`,
    ...
    // instance between attempts.
    {
      ...
      for (let i = 0; i < 12; i++) {
        try {
          ...
          (0, child_process_1.spawnSync)(
            "cmd",
            ["/c", "taskkill", "/F", "/IM", "vale-agent.exe"],
            { stdio: "ignore" },
          );
          try {
    ...
    if (fs.existsSync(PW_ZIP)) {
      ...
      sh(
        `powershell -NoProfile -Command "Expand-Archive -Force -Path '${(0, exports.psq)(PW_ZIP)}' -DestinationPath '${(0, exports.psq)(COMPONENTS_DIR)}'"`,
      );
    ...
    const nodeWhich = (0, child_process_1.spawnSync)("where", ["node"], {
      encoding: "utf8",
    });
    const nodePath =
    ...
    fs.writeFileSync(
      ...
      startDesktopPs((0, exports.psq)(DESK_DIR)).join("\r\n"),
      "utf8",
    );
    ...
    const regRes = ps(
      bootTaskPs(
        (0, exports.psq)(EXE_DST),
        (0, exports.psq)(CFG_FILE),
        true,
      ).join("; "),
    );
    ...
    try {
      ...
      const ureg = ps(
        uninstallRegBodyPs((0, exports.psq)(DIR), pkgVer).join("; "),
      );
      console.log(
  ...
  status() {
    ...
    const out =
      (0, child_process_1.spawnSync)(
        "tasklist",
        ["/FI", "IMAGENAME eq vale-agent.exe"],
        {
          encoding: "utf8",
        },
      ).stdout || "";
  ...
  autostart(args) {
    ...
    for (const t of exports.BOOT_TASKS) {
      ...
      const r = (0, child_process_1.spawnSync)(argv[0], argv.slice(1), {
        stdio: "inherit",
      });
      if (!r || r.status !== 0) {
        console.error(
  ...
  async rollback(args) {
    ...
    const head = (0, child_process_1.spawnSync)(
      "curl",
      [
        "-s",
        "-o",
    ...
    const inst = (0, child_process_1.spawnSync)(
      npmCmd,
  ...
  uninstall(args) {
    ...
    for (const legacy of ["C:\\vale-agent", "D:\\vale-agent"]) {
      if (legacy !== DIR && fs.existsSync(legacy)) {
        ...
        sh(
          `powershell -NoProfile -Command "Remove-Item -LiteralPath '${(0, exports.psq)(legacy)}' -Recurse -Force -ErrorAction SilentlyContinue; Start-Sleep -Milliseconds 500; Remove-Item -LiteralPath '${(0, exports.psq)(legacy)}' -Recurse -Force -ErrorAction SilentlyContinue"`,
        );
    ...
    switch (sub) {
      case "status": {
        ...
        const out =
          (0, child_process_1.spawnSync)(
            "tasklist",
            ["/FI", "IMAGENAME eq cloudflared.exe"],
            { encoding: "utf8" },
          ).stdout || "";
        console.log(
      ...
      case "start": {
        ...
        const ch = (0, child_process_1.spawn)(
          cf,
          ["tunnel", "--config", cfg, "run"],
