...
describe("buildRecipe", () => {
  ...
  it("stamps which session the commands were run against", () => {
    const r = buildRecipe(cleanPath, {
      name: "x",
      sessionKind: "serial",
      sessionLabel: "192.168.1.1",
    });
  ...
  it("tags the entry so a client can find it, and prefixes the title", () => {
    ...
    expect(buildRecipe(cleanPath, { name: "Recipe: ONU check" }).title).toBe(
      "Recipe: ONU check",
    );
  });

  it("falls back rather than producing an empty title", () => {
    expect(buildRecipe(cleanPath, { name: "   " }).title).toBe(
      "Recipe: untitled",
    );
  });
  ...
  it("never claims to run itself", () => {
    ...
    expect(r.content).not.toMatch(
      /automatic|auto-run|will execute|replays? automatically/i,
    );
  });
});
...
describe("recipeWarnings / suggestedTitle", () => {
  it("names only the problems that exist", () => {
    ...
    expect(recipeWarnings([step({ state: "fail" })])).toEqual([
      "1 step failed",
    ]);
    expect(
      recipeWarnings([step({ state: "fail" }), step({ state: "fail" })]),
    ).toEqual(["2 steps failed"]);
    expect(recipeWarnings([step({ state: "running" })])).toEqual([
...
describe("PathView recipe flow", () => {
  ...
  it("saves to the SHARED device memory under the recipe tag", async () => {
    render(
      <PathView events={session()} sessionKind="pty" sessionLabel="shell" />,
    );
  ...
  it("says the path will not finish cleanly BEFORE the operator saves it", () => {
    ...
    fireEvent.click(screen.getByText("Save as recipe"));
    expect(document.querySelector(".path-recipe-warn")!.textContent).toContain(
  ...
  it("states plainly that a recipe does not run itself", () => {
    render(<PathView events={session()} />);
    fireEvent.click(screen.getByText("Save as recipe"));
    expect(document.querySelector(".path-recipe-hint")!.textContent).toMatch(
...
describe("recipes keep the WHY", () => {
  ...
  it("carries the session goal and each step's reasoning", () => {
    ...
    expect(r.content).toContain(
      "why: check whether the ONU is actually online",
    );
    expect(r.content).toContain(
      "instead of: reset the ONU | check the OLT uplink",
    );
  });
