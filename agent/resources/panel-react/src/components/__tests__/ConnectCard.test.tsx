...
const TOKEN = "d3adb33fdeadbeefd3adb33fdeadbeefd3adb33fdeadbeefd3adb33fdeadbeef";
...
const SPEC = {
  plugins: [
    { name: "terminal", tools: [
      { name: "terminal_execute" }, { name: "terminal_open" }, { name: "terminal_read" },
    ] },
    { name: "system", tools: [{ name: "system_file_list" }, { name: "system_process_kill" }] },
...
beforeEach(() => {
  ...
  mockCallApi.mockImplementation((path: string) => {
    ...
    if (path === "/api/status") return Promise.resolve({ ok: true, version: "1.2.307", live_sessions: 2 });
...
describe("ConnectCard", () => {
  it("builds the endpoint from the LIVE host, not a baked-in one", async () => {
    ...
    expect(snippet, "must not hardcode loopback").not.toContain("127.0.0.1:18080");
  ...
  it("masks the device token until it is explicitly revealed", async () => {
    ...
    expect(snippet(), "mask must be an obvious placeholder").toContain("<your-device-token>");

    fireEvent.click(screen.getByText("Reveal token"));
    await waitFor(() => expect(snippet()).toContain(TOKEN));
    expect(snippet(), "placeholder must be gone once revealed").not.toContain("<your-device-token>");
  ...
  it("never puts the credential in a URL (ADR 0004) — for EVERY client", async () => {
    ...
    fireEvent.click(screen.getByText("Reveal token"));
    await waitFor(() =>
      expect(document.querySelector(".connect-snippet")!.textContent).toContain(TOKEN),
    );
    const snippet = () => document.querySelector(".connect-snippet")!.textContent!;
    ...
    for (const tab of ["DSH", "Claude Code", "curl (a quick check)"]) {
      ...
      expect(snippet(), `${tab}: token must not ride a query string`).not.toMatch(/[?&]token=/);
      ...
      expect(snippet(), `${tab}: token must ride an Authorization header`).toContain("Authorization");
      expect(snippet(), `${tab}: header must be a Bearer credential`).toContain("Bearer");
    }
  });
  ...
  it("gives each client its own config shape", async () => {
    ...
    expect(snippet(), "Claude Code uses the mcp add CLI").toContain("claude mcp add");
  ...
  it("counts the tool surface from the live spec, not a baked-in list", async () => {
    ...
    expect(families.some((f) => f?.includes("Terminal") && f.includes("3"))).toBe(true);
    expect(families.some((f) => f?.includes("System") && f.includes("2"))).toBe(true);
  ...
  it("degrades quietly when the spec cannot be read", async () => {
    ...
    expect(document.querySelector(".connect-snippet")!.textContent).toContain("/mcp");
  });
});
