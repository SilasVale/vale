...
let embeddedGuest: Electron.WebContents | null = null;
function trackEmbeddedGuest(wc: Electron.WebContents): void {
  embeddedGuest = wc;
  wc.on("destroyed", () => { if (embeddedGuest === wc) embeddedGuest = null; });
  ...
  wc.setWindowOpenHandler(({ url }) => {
    ...
    if (safe && safe !== "about:blank") {
      try { wc.loadURL(safe); } catch { /* navigations surface via did-fail-load */ }
    }
    return { action: "deny" };
  });
}
ipcMain.handle("embedded-browser:guest-attached", (e, guestId: number) => {
  ...
  try {
    ...
    if (wc) trackEmbeddedGuest(wc);
    return { ok: true };
  } catch { return { ok: false }; }
});
ipcMain.handle("embedded-browser:state", (e) => {
  if (!frameOk(e)) return { ok: false, error: "forbidden frame" };
...
ipcMain.handle("desktop:get-auto-launch", async (e) =>
  frameOk(e) ? { enabled: await autoLaunchTaskExists() } : { ok: false, error: "forbidden frame" });
