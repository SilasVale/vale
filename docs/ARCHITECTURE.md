...
| agent vale-command-core | the contract crate (Plugin/ToolDef/Config/EventBus); canonical import `vale_agent_core::`; tokio-util CancellationToken is the MCP layer's vocabulary (kept, documented). SOLID R11/12: ensure_token table, ToolHandler blanket-impl + cancellable-default pins. SOLID R107: `GATEWAY_DISPATCHED_CODES` pins the three error codes the gateway dispatches on by literal — the boundary is NOT compiled together, and the gateway's own suite never runs the agent, so a rename here degrades a client-visible failure class with every gate green | error.rs (R107) |
| agent paths.rs | single path-resolution truth, OnceLock-cached (boot-invariant) | 584c7669 |
| agent epoch helpers (lib.rs) | `unix_now()`/`now_millis()` are the two epoch stamps, one WORD apart at a call site and 1000× apart in value — the classic silent bug. R115 completed the consolidation its own doc had falsely claimed (the playwright plugin kept a byte-identical `now_ms` plus two inline copies) and made the UNIT CONTRACT machine-checked by MAGNITUDE bound, not by comparing a clock reading. Motivated by a real latent risk: `started_unix` is seconds, echoed to the model, and compared NOWHERE today | now_helpers tests (R115) |
| agent module map (AGENTS.md + CLAUDE.md) | the two guides' module map is a CHECKED claim, not prose (R114). It had rotted: five real `src/` modules were absent (`text.rs`/`jsonl.rs` from R105/R111, plus `register.rs`/`tunnel.rs`/`winmain.rs`), `paths.rs` was mentioned only in prose and never as an entry, and the map's code fence was never closed in either file. `tests/module_map.rs` now asserts every `src/` module is an entry in BOTH maps, no entry names a deleted module, and the two guides agree. It checks NAMES, not accuracy — stated in the header | tests/module_map.rs (R114) |
| agent boot-path panic surface | every boot module documents "never fatal" (migrate_layout_v2 "Never fails the boot", main() "never fatal") but that was prose. The migration runs FIRST in main(), BEFORE tracing init, so a panic there = a device that never starts and leaves no log (the 1.2.223 dark-device class). R110 removed the boot path's only panic surface and added `tests/boot_surface.rs`, which scans the six boot modules for the panic family. The scanner is brace- and string-literal-aware (test modules it must skip contain `format!("{{{{{{")`) and is itself pinned by planted-panic / after-a-test-module / line-number tests. Header states what it does NOT cover (indirect panics, block comments, hand-maintained file list) | tests/boot_surface.rs (R110) |
...
| agent terminal ToolCtx | the terminal tool builders' shared runtime state, as a PARAMETER OBJECT (R112). Before it, `build()` threaded seven params by hand and three builders took five each; the terminal tools keep gaining shared state (`buffer_limit` round-68, `jobs`, `diag`), so every addition re-churned every signature. `ToolCtx` also owns `jobs`, whose two consumers (the executor and `terminal_jobs`) are the pair review #2 required to share ONE map. NOT imposed on single-dep builders — `tool_read(&OutputBuf)` / `tool_diag_read(&DiagStore)` keep focused signatures | ctx.rs (R112) |
...
| agent exec result cap | `bounded_append` is the session wait loop's single choke point for result growth. Extracted (R100) from the 622-line `tool_execute` closure, which had accumulated three incidents against this one concern — round-105 (uncapped growth OOM'd the agent on `yes`), round-113 (a single oversized chunk bypassed the cap until the NEXT append) and two char-boundary panics (round-106 drain; review #1 slice start) that abort the loop PAST `term_release_execute` and wedge the session busy flag forever. Pure, so both boundary walks are unit-pinned and mutation-proven. The wait-loop STATE MACHINE stays inline by design — its rules read live session state and the 50 ms cadence | exec.rs header (R100) |
...
| agent kill-tree policy | `signal_tree(pid, force)` + `wait_for_exit(child, patience)` (R106) own "how a timed-out command's process tree is terminated". Four `#[cfg]`-gated signal blocks and two copies of the same exit-poll loop were inline in `execute_local`. The arms are never compiled together (Linux test vs `cargo xwin`), so duplication here is invisible to every other platform's gate — the strongest case in this crate for a single owner. Its round-55 contract ("kill the TREE, not just the shell") now has its first coverage, driving real processes and asserting the signal reaches a backgrounded grandchild | exec.rs header (R106) |
| agent src/jsonl.rs | append-only JSONL hygiene (R111): `prepare_append` = "write the version header into an empty file, else terminate a torn final line". Two consumers (the per-session audit trail + the memory store) each had their own copy and each documented the same incident — a fragment without its newline fuses onto the next record, and in the audit trail that once swallowed the "interrupted" recovery marker so a crashed command read back as FINISHED. Promoted under the foundation rule (two real consumers, an incident lesson to pin, no env coupling). Header content and the open handle stay caller-owned | jsonl.rs header (R111) |
| agent src/text.rs | byte-budget text clipping, promoted R105 from EIGHT hand-written sites across six files (two different idioms). The naive version has three recorded incidents in this crate — round-68 (panicked the session drainer and WEDGED the session), rounds 110/111 (same class in the diag writer) and an audit-HIGH remote-body slice at byte 80. `boundary_at_or_below` (index, for callers reporting dropped bytes) + `clip` (borrowed slice). Truncation SUFFIX wording stays per-caller deliberately: model-facing output marks `…`, the audit trail marks `…[truncated N bytes]` | text.rs header (R105) |
| agent web/parse.rs | request-body semantics for the two settings endpoints (R104): `json_body` owns the 400 `invalid_params` envelope (error TEXT stays a caller argument — the endpoints' wordings differ deliberately), `optional_trimmed_string` owns "trimmed, blank ⇒ unset" (5 call sites; the rule carries the clobber-`buffer_mb` and unbind-gateway incidents, so absent vs cleared vs blank must stay distinguishable). Pure, so the truth table is unit-tested without a server. Fixes a latent desync: `api_gateway_connect` computed the reported and persisted binding from two independent copies — now one evaluation feeds both | web/parse.rs header (R104) |
| agent router composition | `mcp::bind` assembles `nest_service("/mcp", TokenGate) + fallback_service(WebPanel)` — the ONLY place the routing/auth structure is defined, and it had no test: R102 pinned `handle_request` directly, which a routing edit never touches. `tests/router_auth_integration.rs` (R103) drives a real server over real HTTP and pins all three branches: /mcp gated by its Tower layer, the fallback reaching the web gate (GET+POST), and the public surfaces serving. Both failure directions mutation-proven | tests/router_auth_integration.rs (R103) |
| agent web/ routing split | `handle_request` mixes three concerns no longer (R108): `route_pre_dispatch` owns every route decided BEFORE the API pipeline — the public SPA/assets + status page, plus the three streaming routes that self-authenticate — and returns `Option<Response>`. Consequences: R102's "anything reaching the dispatcher is authenticated" is now structural (a property of WHICH function a request lands in, not of statement order), and the public surface is enumerable in one list. 230 → 158 lines. `check_auth` narrowed to `&HeaderMap` (a `&Request<Body>` is not Send/Sync, so holding one across an await poisons the Tower service's future) | mod.rs (R108) |
| agent web auth gate | `handle_request`'s gate is UNCONDITIONAL (R102): the public surfaces above it (status page, panel/desktop SPA, the three auth-checked streaming routes) own their own decisions, so the old `needs_auth` re-classification was provably always true there — and a duplicated classification fails ASYMMETRICALLY (disagreement ⇒ gate skipped ⇒ unauthenticated dispatch). Anything reaching the dispatcher is now authenticated by construction. `every_dispatch_route_is_auth_gated` (20 routes × missing-header + wrong-token) and `deliberately_public_routes_stay_public` pin both halves; both mutation-proven | web/mod.rs header (R102) |
| agent plugins::tool_error | the device-tool FAILURE envelope, promoted R101 from 46 hand-written literals across 4 plugins. Pinning it surfaced an undocumented cross-layer asymmetry now on record: in-band `Ok({"ok":false})` renders at `/api/tools` as `{"ok":true,"result":{"ok":false}}` (outer ok TRUE, no top-level code) while typed `Err(DeviceError)` renders `{"ok":false,"error","code"}` — the gateway's round-58 check therefore classifies only the TYPED family as a failure. Deliberate (the model reads the in-band envelope as content) and pinned on both sides | plugins/mod.rs (R101) |
| agent memory source field | `MemoryRecord.source` is stamped `"unknown"` on every write (R109): `tools::set_source` — `pub`, documented as "called by the MCP layer on handshake" — has no caller repo-wide, so the client-identity capture three doc comments described never happens. The claims are corrected and the gap is PINNED by `records_are_stamped_unknown_until_set_source_is_wired`, which fails with an instructive message the moment someone wires it. Deliberately NOT repaired here: wiring it is a behaviour change needing a product decision about what identity to record | memory/mod.rs test (R109) |
