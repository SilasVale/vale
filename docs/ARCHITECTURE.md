...
| Muse US exit = self-hosted Oracle VPS relay (2026-09-08; was Vercel) | zen geo-locates the EGRESS IP: CF egress 403 (re-verified after placement fixes), Vercel free team paused at 304% of its 10 GB transfer cap (402); the Always-Free ARM box carries only the ~40 ms intra-US hop (, grey-cloud + LE TLS, body-streaming nginx, 5-min watchdog; since 2026-09-08 the box ALSO serves the FULL vercel-proxy relay set (zen/proxy/github/git/gform) as `vrelay` under Node, completing the Vercel retirement — VPS as USER-side front measured strictly worse (tails to 7.9 s); runbook proxies/README.md |
