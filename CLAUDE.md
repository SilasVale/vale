...
Vale = one repo + one front door: `gateway/` (Vale Gate worker), `command/` (Vale Command, Windows), `index/` (Vale Index worker), `docs/`.
...
```bash
./scripts/build.sh command           # Windows cross-compile vale-command + vale-tray (needs cargo-xwin)
...
- **command**: `command/CLAUDE.md` (cargo-xwin cross-compile, feature gating, MCP tool additions, Windows smoke checklist)
...
- **Subproject changes**: verify inside that subdir (command: cargo test/clippy/xwin check; gateway/index: wrangler deploy).
